...
class Solution2 {
	...
	public static Node MakeSet(int vertex){
	    return new Node(vertex, vertex, 0);
    }

    public static void Union(int node1, int node2){
	    Node A = Sets[FindSet(node1)];
	    Node B = Sets[FindSet(node2)];
	    if(A.rank > B.rank)
	        B.parent = A.self;
	    else{
	        A.parent = B.self;
...
class Node{
    ...
    public Node(int self, int parent, int rank){
        this.self = self;
        this.parent = parent;
        this.rank = rank;
    }
