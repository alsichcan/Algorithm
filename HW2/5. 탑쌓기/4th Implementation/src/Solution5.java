		...
		for (int test_case = 1; test_case <= 10; test_case++) {
			...
			for(int block = 1; block < n ; block++){
			    for(int height = H; height > 0; height--){
			        ...
			        if(height + h[block] <= H){
			            Tower_A[height + h[block]] += Tower_B[height] % 1000000;
                    }

			        if(height + h[block] - d[block-1] <= H) {
                        Tower_A[height + h[block] - d[block - 1]] += Tower_A[height] % 1000000;
                        Tower_B[height] += Tower_A[height] % 1000000;
                        Tower_A[height] = 0;
                    } else{
			            Tower_B[height] += Tower_A[height] % 1000000;
			            Tower_A[height] = 0;
			...
			for(int height = 1; height <= H ; height++){
                Answer += (Tower_A[height] + Tower_B[height]) % 1000000;
                Answer %= 1000000;
            }
